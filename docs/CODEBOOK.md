...
| text | Control statement (curated subset; every row checked against the source before external release) | see LIMITATIONS.md §1 |
| verify_flag | `VERIFIED` in this release | records the author's verbatim check of every SP 800-53 row |
...
| tentative_alignment_type | Rule-based bucket from cosine_similarity: Direct (≥0.30) / Partial (≥0.18) / Related (≥0.08) / No Match | 03_align.py; thresholds documented and challengeable, see BUILD_SPEC.md |
...
| cosine_similarity / tentative_alignment_type | The machine's score and preliminary bucket | copied from candidate_crosswalk.csv |
...
| adjudicated_alignment_type | The human adjudicator's final call: Direct / Partial / Related / No Match / Reject | filled in by the author during Step 5 verification |
| adjudicator | Name of the person who made the call | filled in by the author |
| adjudication_rationale | One to two sentences explaining the call | filled in by the author |
| adjudication_date | ISO date of the decision | filled in by the author |
